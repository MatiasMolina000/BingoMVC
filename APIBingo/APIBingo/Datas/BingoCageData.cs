...
namespace APIBingo.Datas
{
    public class BingoCageData
    {
        ...
        {
            var query = "SELECT * FROM BingoCages WITH(NOLOCK) " +
                "WHERE GameId = @gameId;";

            IEnumerable<BingoCageModel> data = await new DBFactoryConnectionService(_connectionFactory)
                .ExecuteGetListObjectAsync<BingoCageModel>(query, new { gameId });
