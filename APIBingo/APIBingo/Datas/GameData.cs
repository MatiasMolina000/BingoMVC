...
namespace APIBingo.Datas
{
    public class GameData
    {
        ...
        {
            var query = "SELECT * FROM Games WITH(NOLOCK) " +
                "WHERE Id = @id;";

            GameModel? data = await new DBFactoryConnectionService(_connectionFactory)
                .ExecuteGetSingleObjectAsync<GameModel>(query, new { id });
        ...
        public async Task<string?> NewGame(GameModel oModel)
        {
            ...
            string? data = await new DBFactoryConnectionService(_connectionFactory)
                .ExecuteInsertSingleAndGetIdAsync(query, oModel);
            if (data == null || data == "0")
            { 
                return null;
            }
        ...
        {
            var query = "UPDATE Games SET StatusId = 2, [End] = GETDATE(), [Status] = 1 " +
                "WHERE Id = @gameId;";

            string? data = await new DBFactoryConnectionService(_connectionFactory)
                .ExecuteInsertSingleStringAsync(query, new { gameId });
        ...
        {
            var query = "SELECT * FROM Games WITH(NOLOCK) " +
                "WHERE UserId = @usreId AND StatusId = 1 AND [Status] = 0;";

            GameModel? data = await new DBFactoryConnectionService(_connectionFactory)
                .ExecuteGetSingleObjectAsync<GameModel>(query, new { usreId });
        ...
        {
            var query = "UPDATE Games SET StatusId = 3, [End] = GETDATE(), [Status] = 1 " +
                "WHERE Id = @gameId;";
